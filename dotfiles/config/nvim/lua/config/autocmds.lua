...
-- Quit Neovim cleanly: close sidebar/dashboard/special windows
  ...
  callback = function()
    ...
    for _, win in ipairs(vim.api.nvim_list_wins()) do
      if vim.api.nvim_win_get_config(win).relative == "" then
        ...
        -- Skip file tree, dashboard, notify, terminal, and other special buffers
    ...
    if #real_wins <= 1 then
      for _, win in ipairs(vim.api.nvim_list_wins()) do
        if vim.api.nvim_win_get_config(win).relative == "" then
          ...
          local bt = vim.bo[buf].buftype
          local ft = vim.bo[buf].filetype
          if bt == "nofile" or ft == "snacks_dashboard" or ft == "neo-tree" or ft == "neo-tree-popup" or ft == "snacks_notif" then
        ...
        vim.schedule(function()
          local ok = pcall(function()
            require("neo-tree.command").execute({
              source = "filesystem",
              dir = vim.fn.fnamemodify(target, ":p"),
              action = "focus",
            })
          end)
          if not ok then
